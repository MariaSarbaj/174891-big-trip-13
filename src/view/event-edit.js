...
const createEditPointTemplate = (point) => {
  ...
  const renderPhotos = (pictures) => {
    return pictures.map((picture) => {
      return (
        `<img class="event__photo" src="${picture}" alt="Event photo">`.trim());
    })
      .join(`\n`);
  };
  ...
  const typesList = renderTripTypesList(EVENT_TYPES.slice(0, 7));
  const eventPhotos = renderPhotos(destination.pictures);
  const eventOffers = renderOffers(offers);
